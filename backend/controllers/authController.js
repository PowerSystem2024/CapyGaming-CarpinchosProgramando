...
try {
  ...
  // Eliminar el usuario insertado si falla la generación del token
  try {
    await pool.query('DELETE FROM usuario WHERE dni = $1', [newUser.rows[0].dni]);
    console.log("Usuario eliminado debido a error al generar token.");
  } catch (deleteError) {
    console.error("Error eliminando usuario tras fallo en JWT:", deleteError);
  }
